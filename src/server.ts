...
export function buildServer(): McpServer {
  ...
  server.registerTool(
    ...
    async ({ name, description }) => run(() => ops.registerProject(name, description)),
  );
  ...
  server.registerTool(
    ...
    async ({ project, category, description }) => run(() => ops.defineCategory(project, category, description)),
  );
  ...
  server.registerTool(
    ...
    async ({ project, category, chapter, description }) =>
      run(() => ops.defineChapter(project, category, chapter, description)),
  );
  ...
  server.registerTool(
    ...
    async ({ project, category, chapter, topic, description }) =>
      run(() => ops.defineTopic(project, category, chapter, topic, description)),
  );
  ...
  server.registerTool(
    ...
    async ({ project }) => run(() => ops.getProjectMap(project)),
  );
  ...
  server.registerTool(
    "search_docs",
    {
      ...
      description:
        "Full-text search across all doc bodies (fuzzy + prefix matching, relevance-ranked). " +
        "Use it when you don't know which topic covers something — it returns the best-matching topics " +
        "with a snippet each, so you can read_doc exactly the right one. " +
        "Scope to one project when you know it; otherwise all projects are searched.",
      inputSchema: {
        query: z.string().describe("Free-text query, e.g. 'grid column resize'."),
        project: z.string().optional().describe("Optional: limit the search to one registered project."),
        limit: z.number().int().min(1).max(25).default(8).describe("Maximum number of results (default 8)."),
      },
      annotations: { readOnlyHint: true },
    },
    async ({ query, project, limit }) => run(() => searchDocs(query, project, limit)),
  );
  ...
  server.registerTool(
    ...
    async ({ project, category, chapter, topic, kind, content, mode }) =>
      run(() => ops.writeDoc(project, category, chapter, topic, kind, content, mode)),
  );
  ...
  server.registerTool(
    ...
    async ({ project, category, chapter, topic, kind }) =>
      run(() => ops.readDoc(project, category, chapter, topic, kind)),
  );
